...
from estimators import estimate_with_noise_1, estimate_with_noise_2, estimate_with_noise_3
...
def get_characteristics_new(trajectory):
    trajectory_x = trajectory[:,0]
    trajectory_y = trajectory[:,1]
    return CharacteristicFive(trajectory_x, trajectory_y, None, 2,
                            percentage_max_n=0.1).data
...
class CharacteristicBase:
    ...
    def __init__(self, x, y, z, dim, percentage_max_n):
        """
...
class Characteristic(CharacteristicBase):
    ...
    def __init__(self, x, y, z, dim,percentage_max_n=1,):
        """
        :param x: list, x coordinates
        :param y: list, y coordinates
        :param y: list, z coordinates
        ...
        CharacteristicBase.__init__(self,x, y, z, dim, percentage_max_n)

        self.D = None# It will be initialized later. self.get_diffusion_coef()
...
class CharacteristicFour(Characteristic):
    ...
    def __init__(self, x, y, z, dim, percentage_max_n=0.1):
        """
        :param x: list, x coordinates
        :param y: list, y coordinates
        :param y: list, z coordinates
        ...
        Characteristic.__init__(self, x, y, z, dim, percentage_max_n)
...
#        self.D_new = self.estimate_diffusion_coef()
        ...
        traj = np.transpose(np.array([x, y]), (1,0))
        self.D, self.alpha_n_1 = estimate_with_noise_1(traj, 10)
        _, self.alpha_n_2 = estimate_with_noise_2(traj, 10)
        _, self.alpha_n_3 = estimate_with_noise_3(traj, 10)
        self.values = [ self.D, self.alpha,
...
class CharacteristicFive(CharacteristicFour):
    ...
    def __init__(self, x, y, z, dim, percentage_max_n=0.1):
        ...
        CharacteristicFour.__init__(self, x, y, z, dim, percentage_max_n)
    ...
    def get_maximum_test_statistic(self):
        ...
        if self.dim == 1:
            distance = np.array(
                [self.get_displacement(self.x[i], 0, self.x[0], 0) for i in range(1, self.N)])
        elif self.dim == 2:
            distance = np.array(
