...
def save_fov(dir_path, fov, experiment):
    ...
    with open(output_name, 'a') as f:
        for traj_id in fov_results.keys()
            f.write(traj_id)
            f.write(__SEPARATOR)
            functools.reduce(lambda a, b: str(a) + __SEPARATOR + str(b), fov_results[traj_id])
        f.write("\n")

def create_submission(experiments):
    if os.path.isdir(__SUB_WORKING_DIR) 
        os.remove(__SUB_WORKING_DIR)
    ...
    for exp_id in experiments.keys():
        ...
        for fov in range(length(experiments[exp_id])):
            save_fov(dir_path, fov, experiments[exp_id])
    # Zip submission
